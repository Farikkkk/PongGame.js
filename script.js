...
// const paddle1Color = "lightcoral";
// const paddle2Color = "rgb(10, 222, 152)";
...
function computerMove() {
  ...
  const deltaY = targetY - paddle2.y; // Calculate the difference between target and current Y position
  const maxDelta =
    Math.abs(deltaY) > paddleSpeed ? paddleSpeed : Math.abs(deltaY); // Limit the movement to paddleSpeed
  const direction = Math.sign(deltaY); // Determine the direction of movement

  // Move the paddle by the adjusted delta
  paddle2.y += maxDelta * direction;
